...
function writeReplacingPassword(port, timeout, newFirmwarePath, password, progress, completion) {
  var file = temp.openSync({ suffix: ".hex" }); // I know, I know but creating temp files are not gonna take much time

  async.waterfall([
    function (callback) {
      passwordReplacer.replace( path.normalize(newFirmwarePath),
                                "XXXXXXXXXX",
                                password,
                                file.path,
                                progress,
    ...
    function (newPort, callback) {
      ...
      var args = [
        ...
        "-e", // erase whole Flash
        "-U",
        "flash:w:" + path.resolve(file.path) + ":i"
      ];
      var process = avrdude.run(args);
      process.stderr.setEncoding('utf8');
      process.stderr.on("data", function (chunk) {
...
module.exports = {
  ...
  waitPorts: waitPorts,
  readFlash: readFlash,
  writeReplacingPassword: writeReplacingPassword
};
